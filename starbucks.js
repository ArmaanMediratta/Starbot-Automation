...
async function runner(accountIndex){
    ...
    try{
      ...
      try{
        console.log('\x1b[36m%s\x1b[0m', "Task" + accountIndex +': Selecting country(US)');
        const country = await page.$('[name="country"]');
        await country.click();
        await sleep(1500);
        await page.select('select[name="country"]', 'US');
    }catch(e){
        console.log('\x1b[31m%s\x1b[0m', "Task" + accountIndex +': Error selecting country, stopping');
        await browser.close();
        break;
      }
    try{
    ...
    let capStatus;
    try{
        console.log('\x1b[36m%s\x1b[0m', "Task" + accountIndex +': Solving captcha');
        capStatus = await page.solveRecaptchas();
        let realCapStatus = JSON.stringify(capStatus);
        if(realCapStatus === '{"captchas":[],"filtered":[],"solutions":[],"solved":[],"error":null}'){
            console.log('\x1b[34m%s\x1b[0m', "Task" + accountIndex +': No captcha, entering');
        }
        else{
            console.log('\x1b[32m%s\x1b[0m', "Task" + accountIndex +': Solved captcha');
        }
    }catch(e){
        console.log('\x1b[31m%s\x1b[0m', "Task" + accountIndex +': Error solving captcha');
    }
    try{
        ...
        else if(extractedText.includes("already") && extractedText.includes("our") && extractedText.includes("system")){
            console.log('\x1b[31m%s\x1b[0m', "Task" + accountIndex +': Email already registered');
            await browser.close();
            break;
        }
        else{
            console.log('\x1b[31m%s\x1b[0m', "Task" + accountIndex +': Error entering, stopping');
            await browser.close();
            break;
        }
