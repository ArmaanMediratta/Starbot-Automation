...
const accounts = readFileSync("accounts.txt")
  .toString()
  .replace(/\r\n/g, "\n")
  .split("\n");
const proxies = readFileSync("proxies.txt")
  .toString()
  .replace(/\r\n/g, "\n")
  .split("\n");
...
async function runner(accountIndex) {
  for (let j = 0; j < proxies.length; j++) {
    ...
    try {
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Proxy error, stopping"
      );
      browser.close();
    }
    try {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Going to homepage"
      );
      await page.goto(mainLink);
    } catch (e) {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Error going to homepage, stopping"
      );
      await browser.close();
    }
    await sleep(5000);
    try {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Entering first name"
      );
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error inputing first name, stopping"
      );
      await browser.close();
      break;
    }
    try {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Entering last name"
      );
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error inputing last name, stopping"
      );
      await browser.close();
      break;
    }
    try {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Entering email"
      );
      const email = await page.$('[id="email_2"]');
      await email.type(currAcc);
      await sleep(1500);
    } catch (e) {
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error inputing email, stopping"
      );
      await browser.close();
      break;
    }
    try {
      const email = await page.$('[name="confirm_email"]');
      await email.type(currAcc);
      await sleep(1500);
    } catch (e) {
      console.log(
    ...
    try {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Selecting country(US)"
      );
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error selecting country, stopping"
      );
      await browser.close();
      break;
    }
    try {
      const TOS = await page.$('[for="age"]');
      await TOS.click();
      await sleep(1500);
    } catch (e) {
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error inputing sumbitting TOS, stopping"
      );
      await browser.close();
      break;
    }
    ...
    try {
      console.log(
        "\x1b[36m%s\x1b[0m",
        "Task" + accountIndex + ": Solving captcha"
      );
      capStatus = await page.solveRecaptchas();
      let realCapStatus = JSON.stringify(capStatus);
      if (
        realCapStatus ===
        '{"captchas":[],"filtered":[],"solutions":[],"solved":[],"error":null}'
      ) {
        console.log(
          "\x1b[34m%s\x1b[0m",
          "Task" + accountIndex + ": No captcha, entering"
        );
      } else {
        console.log(
          "\x1b[32m%s\x1b[0m",
          "Task" + accountIndex + ": Solved captcha"
        );
      }
    } catch (e) {
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error solving captcha"
      );
    }
    try {
      ...
      await sleep(2000);
      if (extractedText.includes("You're") && extractedText.includes("could")) {
        console.log(
          "\x1b[32m%s\x1b[0m",
          "Task" + accountIndex + ": Successfully entered"
        );
        await browser.close();
        break;
      } else if (
        extractedText.includes("already") &&
        extractedText.includes("our") &&
        extractedText.includes("system")
      ) {
        console.log(
          "\x1b[31m%s\x1b[0m",
          "Task" + accountIndex + ": Email already registered"
        );
        await browser.close();
        break;
      } else {
        console.log(
          "\x1b[31m%s\x1b[0m",
          "Task" + accountIndex + ": Error entering, stopping"
        );
        await browser.close();
        break;
      }
