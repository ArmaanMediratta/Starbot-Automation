...
const mainAccount = readFileSync("top_account.txt")
  .toString()
  .replace(/\r\n/g, "\n")
  .split("\n");
const codes = readFileSync("codes.txt")
  .toString()
  .replace(/\r\n/g, "\n")
  .split("\n");
...
async function main() {
  ...
  try {
    ...
    console.log(
      "\x1b[31m%s\x1b[0m",
      "Task" + accountIndex + ": Captcha error, stopping"
    );
    await browser.close();
  }
  await sleep(3000);
  try {
    ...
    console.log(
      "\x1b[31m%s\x1b[0m",
      "Task" + accountIndex + ": Error logging in, stopping"
    );
    browser.close();
  }
  ...
  while (counter < codes.length) {
    console.log(
      "\x1b[36m%s\x1b[0m",
      "Task" + accountIndex + ": Entering code: " + codes[counter]
    );
    try {
      ...
      console.log(
        "\x1b[32m%s\x1b[0m",
        "Task" + accountIndex + ": Successfully entered code"
      );
      await page.reload();
      await sleep(2500);
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Error sumbitting code"
      );
      await page.reload();
      await sleep(2500);
    }
  ...
  let actAccountBalance = trueAccountBalance + 0.05;
  console.log(
    "\x1b[32m%s\x1b[0m",
    "Task" + accountIndex + ": " + email + " balance is $" + actAccountBalance
  );
