...
async function runner(accountIndex) {
  for (let j = 0; j < proxies.length; j++) {
    ...
    try {
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Cap error, stopping task"
      );
      await browser.close();
      break;
    }
    console.log(
      "\x1b[36m%s\x1b[0m",
      "Task" + accountIndex + ": Checking Status"
    );
    await sleep(5000);
    try {
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Email ineligible"
      );
      browser.close();
      break;
    }
    ...
    await sleep(2000);
    if (
      extractedText.includes("try") &&
      extractedText.includes("again") &&
      extractedText.includes("week")
    ) {
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Not a winner"
      );
      browser.close();
      break;
    } else if (
      extractedText.includes("reason") &&
      extractedText.includes("celebrate") &&
      extractedText.includes("won")
    ) {
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        "Task" + accountIndex + ": Invalid status, stopping"
      );
      browser.close();
      break;
    }
